        ...
        <div className='container-fluid'>
            ...
            <div className='d-flex justify-content-around flex-wrap'>
                {lots.data.map(function (data, index) {
                    if (!data.beforeTime){
                        return (
                            <div key={index} className="card mt-3 justify-content-around">
                                <img className="card-img-top img-lot" src={data.image} alt="Cardimagecap"/>
                                <div className="card-body card-fix">
                                    <h5 className="card-title">{data.name}</h5>
                                    <p className="card-text">{data.description}</p>
                                    <hr/>
                                    <h6 className='card-title'>Стартовая цена: {data.startPrice}</h6>
                                    ...
                                    {data.isYours && <p className="card-text text-info">Вы выигрываете в торгах</p>}
                                    {!data.isYours &&
                                        <p className="card-text text-danger">Вы не выигрываете в торгах</p>}
                                    <p></p>
                                    {data.isGoing && user.isAuth &&
                                        <div className="input-group mb-3">
                                            <div className="input-group-prepend">
                                                <button className="btn btn-outline-secondary" onClick={e => bet(data.id, e)}>Поставить
                                                </button>
                                            </div>
